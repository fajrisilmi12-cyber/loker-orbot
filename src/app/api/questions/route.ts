...
export async function POST(req: Request) {
  try {
    ...
    // 0. Handle File Upload (multipart/form-data)
    if (contentType.includes('multipart/form-data')) {
      ...
      if (!fileName.endsWith('.csv')) {
        return NextResponse.json({
          success: false,
          error: 'Format file tidak valid. Hanya file berekstensi .csv yang diizinkan.'
        }, { status: 400 });
      }
      ...
      if (file.size > MAX_SIZE) {
        return NextResponse.json({
          success: false,
          error: `Ukuran file melebihi batas maksimal 5 MB (ukuran file: ${(file.size / (1024 * 1024)).toFixed(2)} MB)`
        }, { status: 400 });
      }

      const text = await file.text();
      if (!text.trim()) {
        return NextResponse.json({ success: false, error: 'File CSV kosong' }, { status: 400 });
      }

      // Validasi parsing CSV
      try {
        parse(text, {
          columns: false,
          skip_empty_lines: true,
        ...
        const msg = parseErr instanceof Error ? parseErr.message : 'Struktur CSV tidak valid';
        return NextResponse.json({ success: false, error: `Gagal membaca format CSV: ${msg}` }, { status: 400 });
      }

      fs.writeFileSync(CSV_PATH, text.trim() + '\n', 'utf8');
      ...
      return NextResponse.json({
        success: true,
        message: `File ${file.name} berhasil diunggah dan disimpan!`
      });
    }
